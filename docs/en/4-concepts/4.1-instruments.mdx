...
## Example of Form Instrument

Before continuing, consider the following simple form instrument. As you can see, it is just a JavaScript object with a number of properties.
...
Theoretically, an instrument could be defined in the codebase. For instance, the two examples depicted above are both valid instruments.

However, in real-world situations this is quite limiting for interactive instruments. Often, an interactive instrument includes a number of other files, such as CSS, HTML, JavaScript, as well as other assets such as fonts and images.

These files could be considered instrument sources. Instrument sources are the files that define an instrument. For example, suppose we want to add the following files to the interactive instrument depicted above.
